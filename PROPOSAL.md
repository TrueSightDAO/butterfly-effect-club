...
| 2.37 | **`truesight_me/programs/<slug>/manifest.json` is the single per-program registry.** Extends with `roster_sheet_url` (the program's Google Sheet) and `tokenomics_admin_endpoint` (the central GAS URL). The admin panel reads this manifest from `truesight.me/programs/<slug>/manifest.json` at boot — no separate config file lookup needed for routing. Per-program `config.json` shrinks to assets-only (cert template paths). | Gary 2026-05-22 — consolidates the per-program registry into one file |
